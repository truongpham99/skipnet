...
def parse_args():
    ...
    parser.add_argument(
        "arch",
        metavar="ARCH",
        default="cifar10_rnn_gate_rl_110",
        choices=model_names,
    ...
    parser.add_argument(
        "--gate-type", default="rnn", choices=["ff", "rnn"], help="gate type"
    )
    parser.add_argument(
        "--dataset",
        "-d",
    ...
    parser.add_argument(
        ...
        default=10000,
        type=int,
        help="number of total iterations " "(previous default: 64,000)",
    )
    parser.add_argument(
        "--start-iter",
        default=0,
    ...
    parser.add_argument(
        "--resume",
        default=(
            "/nas1-nfs1/home/rds190000/codebase/skipnet_cifar"
            "/save_checkpoints/model_best.pth.tar"
        ),
        type=str,
    ...
    parser.add_argument(
        "--fine_tune", action="store_true", help="fine tune model"
    )
    # rl params
    parser.add_argument(
        "--alpha",
        default=0.1,
        type=float,
        help="Reward magnitude for the " "average number of skipped layers",
    )
    parser.add_argument(
        "--temperature", type=float, default=1, help="temperature of softmax"
    )
    parser.add_argument(
        "--rl-weight", default=0.01, type=float, help="rl weight"
    )
    parser.add_argument(
        "--gamma",
        default=1,
    ...
    parser.add_argument(
        "--restart", action="store_true", help="restart training"
    )
...
def run_training(args, tune_config={}, reporter=None):
    ...
    for i in range(args.start_iter, args.iters):
        ...
        for _, act in gate_saved_actions:
        ...
        total_gate_loss = None
        for (m, action), R in zip(gate_saved_actions, cum_rewards):
            loss = torch.sum(-m.log_prob(action) * (args.rl_weight * R))
            if total_gate_loss is None:
                total_gate_loss = loss
            else:
                total_gate_loss += loss
        ...
        torch.autograd.backward(total_gate_loss + total_loss)
        optimizer.step()

        # measure accuracy and record loss
        (prec1,) = accuracy(output.data, target, topk=(1,))
        # if len(cum_rewards) != 0:
        total_rewards.update(cum_rewards[0].cpu().mean(), input.size(0))
        total_losses.update(total_loss.cpu().mean().item(), input.size(0))
        losses.update(pred_loss.cpu().mean().item(), input.size(0))
        top1.update(prec1.cpu().item(), input.size(0))
        skip_ratios.update(skips, input.size(0))
        total_gate_reward = sum([r.cpu().mean() for r in gate_rewards])
...
def validate(args, test_loader, model):
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        skip_labels["data"].extend(input.detach().cpu().tolist()), 
        skip_labels["label"].extend(target.detach().cpu().tolist()), 
        ts_masks = torch.Tensor()
        for mask in masks:  
            ts_masks = torch.cat((ts_masks, mask.detach().cpu())) 
        ts_masks = ts_masks.reshape((-1, len(input)))
        ts_masks = ts_masks.T.tolist()
        skip_labels["skips"].extend(ts_masks)
    ...
    print(len(skip_labels["data"]), len(skip_labels["label"]), len(skip_labels["skips"]))
    return top1.avg, cp, skip_labels
...
def test_model(args):
    ...
    _,_,train_skip_labels = validate(args, train_loader, model)
    import pickle as pkl
    with open('results/train_skips.pkl', 'wb') as f:
        pkl.dump(train_skip_labels, f)
    ...
    _,_,test_skip_labels = validate(args, test_loader, model)
    with open('results/test_skips.pkl', 'wb') as f:
        pkl.dump(test_skip_labels, f)
