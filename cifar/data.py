...
def prepare_train_data(
    dataset="cifar10", batch_size=128, shuffle=True, num_workers=4
):
    if "cifar" in dataset:
        ...
        transform_extra = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Normalize(
                    (0.4300, 0.4284, 0.4427), (0.1963, 0.1979, 0.1995)
                ),
            ]
        )
...
def prepare_test_data(
    dataset="cifar10", batch_size=128, shuffle=False, num_workers=4
):
    if "cifar" in dataset:
        ...
        transform_test = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Normalize(
                    (0.4524, 0.4525, 0.4690), (0.2194, 0.2266, 0.2285)
                ),
            ]
        )
        testset = torchvision.datasets.__dict__[dataset.upper()](
